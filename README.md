...
### 자바스크립트 변수, 함수 주의사항
* **지역변수 (block-scope)**
* 함수 내부에서 선언된 변수 / 함수 내에서만 접근 가능
* **전역변수 (global-scope)**
* 함수 외부에서 선언된 변수 / 스크립트 전체 어디서든 가능
* *스코프 설정했냐고 물으면 이거 얘기하는거*
...
* `document` 본문 최상위 객체 대상으로 속성, 메소드 이용, 생략할 수 있다.
### DOM 선택시 자주 사용하는 메서드(함수) 종류
...
## 자바스크립트 연산자
* `=` 대입연산자
* 새로운 값이 대입될 때 기존 값은 사라지고 새로운 값이 그 자리를 차지한다.
* `var a = 10`
* `a = 20` //최종 a의 결과는 20
* `+=` 복합대입연산자
...
* <=	a<=b	a가 b보다 작거나 같다
## 논리연산자
* `(조건a)&&(조건b)` 좌우 조건이 모두 참일 경우 실행
* `(조건a)||(조건b)` 좌우 조건이 중 하나만 참이어도 실행
## 조건문
* 1단 조건문
* `if( 조건식 ){참일 경우 실행결과}`
* 2단 조건문
...
* if 중괄호의 바로 뒤에 붙여써야함
* 조거식 작성 시 주의사항 > 참, 거짓으로 결과가 구분되는 데이터를 확인하고 작성해야한다.
### falsy로 구분되는 값
* false, null, undefined, NaN, 0, ""
### truthy로 구분되는 값
* falsy가 아닌 모든 값, [](빈 배열), {}(빈 객체)