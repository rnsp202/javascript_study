...
//배열로 구성된 확인정보로 실제 디자인 및 동적기능 등을 넣으니 적용할 때는 인덱스 및 반복문 등으로 개별 접근을 해야한다.
console.log(nav_a[0])
//(위) [index]를 사용함으로써 개별접근을 해 다양한 기능 수행이 가능하지만 개수가 많을 때 반복입력을 해야 해 비효율적. 이 문제를 해결하는게 '반복문'이다.
//for (ES5)
//초기값은 DOM index 시작 값 0
//i는 초기값 0으로 시작해서 nav_a의 실제 개수보다 4 작을 때까지 반복문 결과를 실행시키면서 증감해라
// for(let i=0; i<4; i++){console.log(nav_a[i])}
console.log('---------------------------')
//for~in  (ES6)
//for~in 뒤로 2개 이상의 배열값을 가지고 있는 객체를 삼입하면 그 대상이 자동으로 let i로 생성한 반복계수에 모두 대입돼서 반복처리된다.
...
객체 인덱스 배열 변수는 의미있는 이름으로 자유설정가능
